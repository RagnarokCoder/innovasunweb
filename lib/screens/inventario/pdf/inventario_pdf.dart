...
Future<void> generateInventoryPdf(StateSetter setter) async {
...
//Draw the image to the PDF page.
  ...
  await saveAndLaunchFile(bytes, 'reporteinventario.pdf');
  setter(() {
    //isPdfLoding = false;
  });
}
...
PdfLayoutResult drawHeader(PdfPage page, Size pageSize, PdfGrid grid) {
  ...
  //Draw string
  page.graphics.drawString('''Reporte de Inventario''', title,
      brush: PdfBrushes.black,
      bounds: const Rect.fromLTWH(190, 70, 300, 55),
      format: PdfStringFormat(
          alignment: PdfTextAlignment.left,
          lineAlignment: PdfVerticalAlignment.bottom));
  page.graphics.drawString('''''', contentFontN,
      brush: PdfBrushes.black,
...
PdfGrid getGrid() {
  ...
  headerRow.cells[2].value = 'Ubicación';
  headerRow.cells[3].value = 'Descripción';
  headerRow.cells[4].value = 'Fecha';
  //Add rows

  for (var i in inventarioAll.keys) {
    addProducts(
        inventarioAll[i]['nombre'],
        inventarioAll[i]['cantidad'],
        inventarioAll[i]['categoria'],
        inventarioAll[i]['descripcion'],
        inventarioAll[i]['fecha'],
        grid);
  }
...
void addProducts(String name, dynamic cantidad, String ubicacion, descripcion,
    fecha, PdfGrid grid) {
  final PdfGridRow row = grid.rows.add();
  row.cells[0].value = name;
  row.cells[1].value = cantidad.toString();
  row.cells[2].value = ubicacion;
  row.cells[3].value = descripcion;
  row.cells[4].value = fecha.toDate().toString().split(" ")[0];
}
