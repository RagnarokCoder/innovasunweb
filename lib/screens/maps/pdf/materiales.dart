// ignore_for_file: avoid_web_libraries_in_flutter, unused_import, depend_on_referenced_packages, unused_local_variable
...
Future<void> generateMateriales(
    StateSetter setter, Map<dynamic, dynamic> instalacion) async {
  ...
  //Generate PDF grid.
  final PdfGrid grid = getGrid(instalacion);

  //Draw the header section by creating text element
  final PdfLayoutResult result = drawHeader(page, pageSize, grid, instalacion);
...
//Draw the image to the PDF page.
  ...
  //Draw grid
  drawGrid(page, grid, result, instalacion);
  //Add invoice footer
  ...
  await saveAndLaunchFile(bytes, 'materiales.pdf');
  setter(() {
    isMateriales = false;
  });
}
...
PdfLayoutResult drawHeader(PdfPage page, Size pageSize, PdfGrid grid,
    Map<dynamic, dynamic> instalacion) {
  //Draw rectangle
  ...
  //Draw string
  page.graphics.drawString('''Entrega de Material''', title,
      brush: PdfBrushes.black,
      bounds: const Rect.fromLTWH(180, 70, 300, 55),
  ...
  String creacion =
      DateFormat('dd-MM-yyyy').format(instalacion['fecha'].toDate());
  String datosPago = "Observaciones";
  String detalle2 =
      '''_____________________________\t\t\t\t\t\t\t\t\t\t\t\t_______________________________\n                      Entregado:                                                                                              Recibido: ''';
  final Size contentSize = contentFont.measureString(datosPago);
  // ignore: leading_newlines_in_multiline_strings
  String detalleProveedor =
      '''Fecha: $creacion\nMunicipio/Dirección: ${instalacion['comunidad']} / ${instalacion['direccion']}\nInstalador: ${instalacion['usuario']}''';

  PdfTextElement(text: datosPago, font: contentFontN).draw(
      page: page, bounds: Rect.fromLTWH(30, 600, 480, pageSize.height - 120))!;
...
Future<void> drawGrid(PdfPage page, PdfGrid grid, PdfLayoutResult result,
    Map<dynamic, dynamic> instalacion) async {
...
PdfGrid getGrid(Map<dynamic, dynamic> instalacion) {
  //Create a PDF grid
  ...
  headerRow.cells[0].value = 'Material';
  headerRow.cells[0].stringFormat.alignment = PdfTextAlignment.center;
  headerRow.cells[1].value = 'Cantidad';
  //Add rows

  Map<dynamic, dynamic> productos = {};
  productos = instalacion['materiales'] as Map;
  debugPrint(productos.toString());
  for (var i in productos.keys) {
    addProducts(i, productos[i], 0, grid);
  }
