...
PdfLayoutResult? drawHeader(PdfPage page, Size pageSize, PdfGrid grid) {
  ...
  String nombre = '''Corte del día''';
  String footerContent =
      // ignore: leading_newlines_in_multiline_strings
      '''''';
  for (var i in rInstalaciones.keys) {
    total += (rInstalaciones[i]['cantidad']);
  }
  String monedaS =
      ''' \r\n\r Reporte de instalaciones de por fecha ${rangoA.toString().split(" ")[0]} a ${rangoB.toString().split(" ")[0]} \nTotal: ${f.format(total)}''';
...
PdfGrid getGrid(Size size) {
  ...
  headerRow.cells[7].value = 'Usuario';

  final sortedKeysDesc = SplayTreeMap<String, dynamic>.from(
      rInstalaciones, (keys2, keys1) => keys2.compareTo(keys1));
  //Add rows
  debugPrint(rInstalaciones.length.toString());
  for (var i in rInstalaciones.keys) {
    addProducts(
        i,
