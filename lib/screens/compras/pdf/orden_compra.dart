...
dynamic bytspdf;
Future<void> generateCompra(
    StateSetter setter, Map<dynamic, dynamic> compra) async {
  ...
  //Generate PDF grid.
  final PdfGrid grid = getGrid(compra);

  //Draw the header section by creating text element
  final PdfLayoutResult result = drawHeader(page, pageSize, grid, compra);
...
//Draw the image to the PDF page.
  ...
  //Draw grid
  drawGrid(page, grid, result, compra);
  //Add invoice footer
  ...
  await saveAndLaunchFile(bytes, 'compra.pdf');
  setter(() {
    isLoadPdf = false;
    bytspdf = bytes;
    debugPrint(bytspdf.toString());
  });
}

//Draws the invoice header
PdfLayoutResult drawHeader(
    PdfPage page, Size pageSize, PdfGrid grid, Map<dynamic, dynamic> compra) {
  //Draw rectangle
  ...
  //Draw string
  page.graphics.drawString('''Orden de compra''', title,
      brush: PdfBrushes.black,
      bounds: const Rect.fromLTWH(210, 0, 200, 55),
  ...
  String variableText = '${compra['uuid']}';
  ...
  String noproveedor = "Proveedor: ";
  String noproovedorv = "${compra['correo']}";
  ...
  nprovva.draw(
    page: page,
    bounds: Rect.fromLTWH(85, 120, pageSize.width - (contentSize.width + 30),
        pageSize.height - 120),
  );
...
Future<void> drawGrid(PdfPage page, PdfGrid grid, PdfLayoutResult result,
    Map<dynamic, dynamic> compra) async {
  ...
  page.graphics.drawString(
      compra['isIva'] == true
          ? "\$${f.format((compra['total'] / 1.16))}"
          : "\$${f.format((compra['total']))}",
      PdfStandardFont(PdfFontFamily.helvetica, 9, style: PdfFontStyle.bold),
  ...
  page.graphics.drawString(compra['isIva'] == true ? 'Iva: ' : '',
      PdfStandardFont(PdfFontFamily.helvetica, 9, style: PdfFontStyle.bold),
      bounds: Rect.fromLTWH(quantityCellBounds!.left, result.bounds.bottom + 20,
          quantityCellBounds!.width, quantityCellBounds!.height));
  page.graphics.drawString(
      compra['isIva'] == true ? "\$${f.format((compra['total'] * .16))}" : "",
      PdfStandardFont(PdfFontFamily.helvetica, 9, style: PdfFontStyle.bold),
      bounds: Rect.fromLTWH(
          totalPriceCellBounds!.left,
...
PdfGrid getGrid(Map<dynamic, dynamic> compra) {
  //Create a PDF grid
  ...
  productos = compra['carrito'] as Map;
